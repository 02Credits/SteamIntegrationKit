...
int32 USIK_MatchmakingLibrary::AddFavoriteGame(FSIK_AppId AppID, FString IP, int32 ConnPort, int32 QueryPort,
    ...
    return SteamMatchmaking()->AddFavoriteGame(AppID.GetAppID(), Var_Address.Value, ConnPort, QueryPort, Data,
...
void USIK_MatchmakingLibrary::AddRequestLobbyListDistanceFilter(
    ...
    SteamMatchmaking()->AddRequestLobbyListDistanceFilter(
        static_cast<ELobbyDistanceFilter>(LobbyDistanceFilter.GetValue()));
#endif
}
...
void USIK_MatchmakingLibrary::AddRequestLobbyListNumericalFilter(FString KeyToMatch, int32 ValueToMatch,
    ...
    SteamMatchmaking()->AddRequestLobbyListNumericalFilter(TCHAR_TO_ANSI(*KeyToMatch), ValueToMatch,
                                                           static_cast<ELobbyComparison>(ComparisonType.GetValue()));
#endif
}
...
void USIK_MatchmakingLibrary::AddRequestLobbyListStringFilter(FString KeyToMatch, FString ValueToMatch,
    ...
    SteamMatchmaking()->AddRequestLobbyListStringFilter(TCHAR_TO_ANSI(*KeyToMatch), TCHAR_TO_ANSI(*ValueToMatch),
                                                        static_cast<ELobbyComparison>(ComparisonType.GetValue()));
#endif
}
...
bool USIK_MatchmakingLibrary::GetFavoriteGame(int32 GameIndex, FSIK_AppId &AppID, FString &IP, int32 &ConnPort,
    ...
    uint32 Var_Data;
    bool Result = SteamMatchmaking()->GetFavoriteGame(GameIndex, &AppID_t, &Var_IP, &Var_ConnPort, &Var_QueryPort,
                                                      &Var_Data, &Var_Time32LastPlayedOnServer);
    AppID = AppID_t;
...
void USIK_MatchmakingLibrary::GetLobbyChatEntry(FSIK_SteamId LobbyID, int32 ChatID, FSIK_SteamId &OutSteamIDUser,
                                                FString &OutChatEntry,
                                                TEnumAsByte<ESIK_LobbyChatEntryType> &OutEntryType) {
#if (WITH_ENGINE_STEAM && ONLINESUBSYSTEMSTEAM_PACKAGE) || (WITH_STEAMKIT && !WITH_ENGINE_STEAM)
    ...
    // 4) Pull the entry
    int32 BytesRead = SteamMatchmaking()->GetLobbyChatEntry(LobbySteamID, ChatID, &SenderSteamID, Buffer.GetData(),
                                                            Buffer.Num(), &SteamType);
...
bool USIK_MatchmakingLibrary::GetLobbyDataByIndex(FSIK_SteamId LobbyID, int32 DataIndex, FString &Key, FString &Value) {
    ...
    char Var_Value[4096];
    bool Result =
        SteamMatchmaking()->GetLobbyDataByIndex(LobbyID.GetSteamID(), DataIndex, Var_Key, 4096, Var_Value, 4096);
    Key = FString(Var_Key);
...
bool USIK_MatchmakingLibrary::GetLobbyGameServer(FSIK_SteamId LobbyID, FString &ServerIP, int32 &ServerPort,
    ...
    bool Result =
        SteamMatchmaking()->GetLobbyGameServer(LobbyID.GetSteamID(), &Var_ServerIP, &Var_ServerPort, &Var_SteamID);
...
FString USIK_MatchmakingLibrary::GetLobbyMemberData(FSIK_SteamId LobbyID, FSIK_SteamId UserID, FString Key) {
    ...
    return UTF8_TO_TCHAR(
        SteamMatchmaking()->GetLobbyMemberData(LobbyID.GetSteamID(), UserID.GetSteamID(), TCHAR_TO_ANSI(*Key)));
#else
    return "";
#endif
}
...
bool USIK_MatchmakingLibrary::SendLobbyChatMessage(FSIK_SteamId LobbyID, FString Message) {
    ...
    return SteamMatchmaking()->SendLobbyChatMsg(LobbyID.GetSteamID(), UTF8.Get(), ByteLen + 1);
#else
    return false;
#endif
}
...
void USIK_MatchmakingLibrary::SetLobbyGameServer(FSIK_SteamId LobbyID, FString ServerIP, int32 ServerPort,
                                                 FSIK_SteamId SteamID) {
#if (WITH_ENGINE_STEAM && ONLINESUBSYSTEMSTEAM_PACKAGE) || (WITH_STEAMKIT && !WITH_ENGINE_STEAM)
    if (!SteamMatchmaking()) {
        return;
...
void USIK_MatchmakingLibrary::JoinLobby(FSIK_SteamId LobbyId) {
    ...
    SteamMatchmaking()->JoinLobby(LobbyId.GetSteamID());
#else
    return;
#endif
}
